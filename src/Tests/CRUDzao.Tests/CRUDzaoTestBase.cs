...
namespace CRUDzao.Tests
{
    public abstract class CRUDzaoTestBase : AbpIntegratedTestBase<CRUDzaoTestModule>
    {
        ...
        protected void UsingDbContext(Action<CRUDzaoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CRUDzaoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CRUDzaoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CRUDzaoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CRUDzaoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CRUDzaoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CRUDzaoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CRUDzaoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CRUDzaoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CRUDzaoDbContext, Task<T>> func)
        {
            T result;
